...
export class TransactionsController {
  ...
  @ApiOperation({
    ...
    description:
      'Paginated list of import declarations. Filter by matched/unmatched status, broker, or port.',
  })
  ...
  @ApiOperation({
    ...
    description:
      'Returns the transaction with its linked product (if matched) and checker finding (if evaluated), including the duty gap.',
  })
  @ApiOkResponse({ description: 'Transaction detail with product and finding' })
  @ApiNotFoundResponse({ description: 'Transaction not found' })
  async getOne(@Param('id') id: string): Promise<{ data: TransactionDetail }> {
